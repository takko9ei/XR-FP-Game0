...
public class Enemy : MonoBehaviour
{
    ...
    void turnHpDisplayTowardsPlayer()
    {
    ...
    {
        //two attack ways
        //atk - player def, min 1, hp minus this
        //implements in player
        //play attack anim
        //this.GetComponent<Animator>().SetFloat()
        //need cooldown
        //if (coolDown) {
        //    coolDown = false;
    ...
    private void addDeadCount()
    {
        if (!addedCount)
        {
            ...
            {
                case 10:
                    gm.GetComponent<GameManagement>().killEnemyCount[0] += 1;
                    break;
                case 15:
                    gm.GetComponent<GameManagement>().killEnemyCount[1] += 1;
                    break;
                case 18:
                    gm.GetComponent<GameManagement>().killEnemyCount[2] += 1;
                    break;
                default:
                    break;
            }
            if ((gm.GetComponent<GameManagement>().killEnemyCount[0]+ gm.GetComponent<GameManagement>().killEnemyCount[1]+ gm.GetComponent<GameManagement>().killEnemyCount[2])%3 == 0)
            {
                gm.GetComponent<GameManagement>().generateFlag = true;
            }
        }
    }
}
